...
1);this.range=this.roundToPrecision(Math.abs(this.ceilToPrecision(f,d)-this.floorToPrecision(e,d)),d);this.start_value=this.floorToPrecision(e,d);this.zero_value=this.roundToPrecision(Math.abs(e)/this.step,d)},roundToPrecision:function(a,b){var c=Math.pow(10,b);return Math.round(a*c)/c},floorToPrecision:function(a,b){var c=Math.pow(10,b);return Math.floor(a*c)/c},ceilToPrecision:function(a,b){var c=Math.pow(10,b);return Math.ceil(a*c)/c},niceNumber:function(a,b){var c=Math.floor(Math.LOG10E*Math.log(a)),
d;c=c<0?parseFloat(Math.pow(10,c).toFixed(Math.abs(c))):Math.pow(10,c);d=a/c;return(b?d<1.5?1:d<3?2:d<7?5:10:d<=1?1:d<=2?2:d<=5?5:10)*c},roundToOrigin:function(a,b){var c=a;c=Math.pow(10,-(b||1));c=Math.round(a*c)/c;return c>this.min?this.roundToOrigin(a-this.step):c}});
...
datalabels:"",hover_color:"",watermark:false,watermark_location:false,hide_empty_label_grid:false,left_padding:false,label_rotation:0,label_max_size:false,focus_hint:true,min:0,max:null};Object.extend(this.options,this.chartDefaults()||{});Object.extend(this.options,c||{});this.element=a;this.data_sets=Object.isArray(b)?new Hash({one:b}):$H(b);if(this.chartDefaults().stacked===true){this.real_data=this.deepCopy(this.data_sets);this.data_sets=this.stackData(this.data_sets)}this.flat_data=this.data_sets.collect(function(d){return d[1]}).flatten();
...
"stroke-width":this.options.stroke_width+"px"});$A(g).each(function(k,n){var o=k[0],p=k[1];b instanceof Array?this.drawPlot(n,j,o,p,b[n%b.length],g,d,e,f):this.drawPlot(n,j,o,p,b,g,d,e,f)}.bind(this));if(this.options.area&&this.options.stroke_width>0){i=this.paper.path().attr({stroke:b,"stroke-width":this.options.stroke_width+"px"});g.remove(0);g.remove(-1);$A(g).each(function(k,n){this.drawPlot(n,i,k[0],k[1],b,g,d,e,f,true)}.bind(this));this.globalAreaLineSet.push(i)}if(this.options.datalabels&&
...
10)-0.5,parseInt(this.y_padding_top,10))},makeValueLabels:function(a){for(var b=this.label_step,c=this.start_value,d=[],e=0;e<a;e++){c=this.roundValue(c+b,3);d.push(c)}return d},drawMarkers:function(a,b,c,d,e,f){var g=parseInt(this.x_padding_left,10)-0.5+d*b[0],h=this.options.height-this.y_padding_bottom+d*b[1],j=this.paper.path().attr({stroke:this.options.label_color}),i={font:this.options.font_size+'px "Arial"',stroke:"none",fill:this.options.label_color};Object.extend(i,f||{});a.each(function(l){if(this.options.draw_axis&&
(this.options.hide_empty_label_grid===true&&l!==""||this.options.hide_empty_label_grid===false)&&this.options.show_ticks){j.moveTo(parseInt(g,10),parseInt(h,10)+0.5);j.lineTo(parseInt(g,10)+5*b[1],parseInt(h,10)+0.5+5*b[0])}l!==""&&this.paper.text(g+e[0],h-2-e[1],l.toString()).attr(i);g+=c*b[0];h+=c*b[1]}.bind(this))},drawVerticalLabels:function(){for(var a=this.graph_height/this.y_label_count,b=this.options.vertical_label_unit?" "+this.options.vertical_label_unit:"",c=0;c<this.value_labels.length;c++)this.value_labels[c]+=
...
6,j=this.step,i=this.options.stacked?this.graph_height/9:this.graph_height/3;b=this.paper.circle(b,c,this.options.marker_size).attr({"stroke-width":"1px",stroke:this.options.background_color,fill:d,opacity:0});d=this.paper.rect(g,h,j,i).attr({fill:d,"stroke-width":0,stroke:d,opacity:0});e=this.options.datalabels?typeof e=="function"?e.call(this,a,f):e+": "+f:""+f;e+=this.options.vertical_label_unit?" "+this.options.vertical_label_unit:"";var l=this.paper.set();e=this.paper.text(b.attrs.cx,b.attrs.cy-
this.options.font_size*1.5-8,e).attr({"font-size":this.options.font_size,fill:this.options.hover_text_color,opacity:1});f=e.getBBox();c=this.drawRoundRect(e,f,4);f=this.drawNib(e,f,4);l.push(b,c,f,e).attr({opacity:0});this.checkHoverPos({rect:c,set:l,marker:b,nib:f,textpadding:4});this.globalHoverSet.push(l);this.globalBlockSet.push(d);d.hover(function(){l.animate({opacity:1},200)},function(){l.animate({opacity:0},200)})}}});
...
this.bar_padding},calculateStep:function(){this.data_size=this.data_size==1?2:this.data_size;return(this.graph_width-this.options.plot_padding*2-this.bar_padding*2)/(this.data_size-1)},longestDataSetLength:function(){var a=0;this.data_sets.each(function(b){a=b[1].length>a?b[1].length:a});if(Prototype.Browser.IE)a-=1;return a},drawPlot:function(a,b,c,d,e,f,g){b=this.options.height-this.y_padding_bottom-this.zero_value*(this.graph_height/this.y_label_count);var h=this.options.bargraph_lastcolor,j=this.options.bargraph_negativecolor||
...
g=this.x_padding_left+f-0.5,h=this.options.bargraph_lastcolor,j=this.options.bargraph_negativecolor||b;this.datalabel=d;$A(c).each(function(i,l){var k,n=this.options.horizontal_rounded?this.bar_width/2:0,o;k=h&&l===c.length-1?h:i<0?j:b;i=i/this.graph_width*(this.graph_width-f);o=this.paper.rect(g,e,i,this.bar_width,n).attr({fill:k,"stroke-width":0,stroke:k,"stroke-opacity":0});i<0&&o.attr({width:-o.attrs.width}).translate(i,0);if(n){var p=this.paper.set(),m=this.paper.rect(g,e,i-this.bar_width/2,
this.bar_width).attr({fill:k,"stroke-width":0,stroke:k,"stroke-opacity":0});p.push(m,o);i<0&&m.attr({width:-m.attrs.width-this.bar_width}).translate(i+this.bar_width/2,0)}if(this.options.datalabels){var t=this.options.hover_color||k,q=this.paper.set();m=this.datalabel[l].toString();m=this.paper.text(f+i+this.x_padding_left/2,o.attrs.y-this.options.font_size*1.5,m).attr({"font-size":this.options.font_size,fill:this.options.hover_text_color,opacity:1});var u=this.paper.rect(this.x_padding_left,e,this.graph_width,
this.bar_width).attr({fill:k,"stroke-width":0,stroke:k,opacity:0}),r=m.getBBox();i<0&&m.translate(r.width,0);var s=this.drawRoundRect(m,r,4);r=this.drawNib(m,r,4);q.push(s,r,m).attr({opacity:0});this.checkHoverPos({rect:s,set:q,nib:r});this.globalHoverSet.push(q);this.globalBlockSet.push(u);s.attrs.y<0&&q.translate(0,1-s.attrs.y);u.hover(function(){n?p.animate({fill:t,stroke:t},200):o.animate({fill:t,stroke:over_color},200);q.animate({opacity:1},200)},function(){n?p.animate({fill:k,stroke:k},200):
o.animate({fill:k,stroke:k},200);q.animate({opacity:0},200)})}e+=this.step}.bind(this))},drawFocusHint:function(){var a=this.x_padding_left+10,b=this.options.height-this.y_padding_bottom-2.5;this.paper.path().attr({stroke:this.options.label_color,"stroke-width":2}).moveTo(a,b).lineTo(a-5,b+5).moveTo(a-5,b).lineTo(a-10,b+5)},drawVerticalLabels:function(){var a=this.step/2,b=this.options.label_rotation?{"text-anchor":"end",rotation:this.options.label_rotation,translation:"0 "+this.options.font_size/
...
[1,0],a,a,[0,(this.options.font_size+7)*-1])},drawMeanLine:function(a){a=$A(a).inject(0,function(b,c){return c+b})/a.length;a=this.options.bar?a+this.zero_value*(this.graph_height/this.y_label_count):a;this.paper.path().attr(this.options.meanline).moveTo(this.x_padding_left-1+a,this.y_padding_top).lineTo(this.x_padding_left-1+a,this.y_padding_top+this.graph_height)}});
...
Grafico.SparkBar=Class.create(Grafico.SparkLine,{calculateStep:function(){return this.options.width/this.data.length},drawLines:function(a,b){var c=this.options.bargraph_lastcolor,d=this.step>2?this.step-1:this.step,e=d/2,f=this.normalise(0);b.each(function(g,h){var j,i;j=c&&h===b.length-1?c:a;i=this.paper.path().attr({stroke:j,"stroke-width":d});i.moveTo(e,this.options.height-g);i.lineTo(e,this.options.height-f);if(g<f)i.attr({stroke:this.options.bargraph_negativecolor||j});e+=this.step}.bind(this))},
